...
describe("config", () => {
  ...
  it("has docsDir and dbPath as absolute paths", () => {
    assert.ok(path.isAbsolute(config.docsDir), "docsDir should be an absolute path");
...
describe("SYSTEM_PROMPT", () => {
  ...
  it("mentions the RAG domain", () => {
    const lower = SYSTEM_PROMPT.toLowerCase();
    assert.ok(lower.includes("rag") || lower.includes("retrieval-augmented"),
      "prompt must establish the RAG domain");
  });
  ...
  it("instructs structured response format", () => {
    assert.ok(SYSTEM_PROMPT.includes("Summary") && SYSTEM_PROMPT.includes("Reference"),
...
describe("SYSTEM_PROMPT_COMPACT", () => {
  ...
  it("still forbids inventing details", () => {
    assert.ok(SYSTEM_PROMPT_COMPACT.toLowerCase().includes("never invent"));
  });
});
