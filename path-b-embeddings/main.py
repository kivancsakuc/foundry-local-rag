"""
Path B - In-memory embeddings RAG.

This is the Microsoft Learn tutorial code, kept deliberately close to the original:

    https://learn.microsoft.com/en-us/azure/foundry-local/tutorials/tutorial-build-rag-app

Retrieval here is genuine semantic search over neural embeddings, in contrast to
Path A, which scores keyword overlap with TF-IDF and downloads no embedding model.

Nothing is persisted: every start re-embeds the whole document list. Path C fixes
that with SQLite while reusing `cosine_similarity` and `find_relevant` unchanged.
...
def main():
    ...
    print('\nTry: "What programming languages does the SDK support?"')
    ...
    while True:
        ...
        for chunk in chat_client.complete_streaming_chat(messages):
            # The final streaming chunk carries usage data and an EMPTY choices
            # list. The Learn tutorial indexes choices[0] unguarded and therefore
            # crashes with IndexError at the end of every answer.
